...
void learnColumnConfigure(caerModuleData moduleData, int64_t output_neuron_address) {
	...
	int32_t cam_id = 0;
	int64_t post_neuron_addr = output_neuron_address; // int64_t post_neuron_addr = 1 << 10 | 0 << 8 | nsm_winner_neuron_id;
	int32_t num_ex = 0;
	int32_t num_in = 0;

	if (memory.learned_post_synaptic_neuron->buffer2d[output_neuron_address - MEMORY_NEURON_ADDR_OFFSET][0] == 1)
		return;
	else
		memory.learned_post_synaptic_neuron->buffer2d[output_neuron_address - MEMORY_NEURON_ADDR_OFFSET][0] = 1;
				...
				for (post_neuron_id = 0; post_neuron_id < NSM_SIZE; post_neuron_id++) {
					for (pre_row_id = 0; pre_row_id < FEATURE_LAYER_EI_L-1; pre_row_id++) {
						...
						buildSynapseDSNN(moduleData, pre_neuron_addr, post_neuron_addr, virtual_neuron_addr,
							NO_SYNAPSE_ID, real_virtual_synapse, virtual_neuron_addr_enable, 63); //FIRST_CAM_ID
					}
				}
			}
		}
			...
			for (post_col_id = 0; post_col_id < NSM_W; post_col_id++) {
				for (post_neuron_id = 0; post_neuron_id < NSM_SIZE; post_neuron_id++) {
					for (pre_row_id = 0; pre_row_id < FEATURE_LAYER_EI_L-1; pre_row_id++) {
						...
						post_neuron_addr = nsm_state_1[post_row_id][post_col_id][post_neuron_id];
						buildSynapseDSNN(moduleData, pre_neuron_addr, post_neuron_addr, virtual_neuron_addr,
							FAST_EX_SYNAPSE_VALUE, real_virtual_synapse, virtual_neuron_addr_enable, 63); //FIRST_CAM_ID
					}
				}
			}
		}
