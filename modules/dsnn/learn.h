...
//					printf("Count. \n");
	...
	/********************************************************************************/
	if (chip_id == 1 &&
		((core_id == 1 && row_id < NSM_SIZE / 2 * NSM_L && col_id < NSM_SIZE / 2 * NSM_W) ||
		(core_id == 3 && row_id >= NSM_SIZE / 2 * NSM_L * 3 && row_id < NSM_SIZE / 2 * NSM_L * 3 * 2 && col_id < NSM_SIZE / 2 * NSM_W) ||
		core_id == 0 || core_id == 2)) {
		int64_t spike_addr = chip_id << NEURON_CHIPID_SHIFT | core_id << NEURON_COREID_SHIFT | neuron_id;
	...
	if (filtered_input_ready == 1 && nsm_winner_ready == 0) {
		nsm_winner_id = 255; //
		nsm_winner_spikes_num = 0; //
		if (chip_id == 1 && core_id == 1 && row_id < NSM_SIZE / 2 * NSM_L && col_id < NSM_SIZE / 2 * NSM_W) {

			int64_t start_spike_time = ts;
			int64_t current_spike_addr, current_spike_time, delta_time;
			int64_t current_spike_chip_id, current_spike_core_id, current_spike_row_id, current_spike_col_id;
			...
			for (uint64_t current_rd_pointer = (memory.start_rd_pointer_nsm - 1) % SPIKE_QUEUE_LENGTH;
				...
				current_spike_chip_id = (current_spike_addr & NEURON_CHIPID_BITS) >> NEURON_CHIPID_SHIFT;
				current_spike_core_id = (current_spike_addr & NEURON_COREID_BITS) >> NEURON_COREID_SHIFT;
				current_spike_row_id = (current_spike_addr & NEURON_ROWID_BITS) >> NEURON_ROWID_SHIFT;
				current_spike_col_id = (current_spike_addr & NEURON_COLID_BITS) >> NEURON_COLID_SHIFT;

				delta_time = (int64_t) (start_spike_time - current_spike_time) / 1000;

				if (delta_time > 0 && delta_time <= CONSIDERING_PERIOD_CHECK) {
					if (current_spike_chip_id == 1 && current_spike_core_id == 3 &&
						current_spike_row_id >= NSM_SIZE / 2 * NSM_L * 3 &&
						current_spike_row_id < NSM_SIZE / 2 * NSM_L * 3 * 2 &&
						current_spike_col_id < NSM_SIZE / 2 * NSM_W) {
						nsm_winner_ready = 0;
						break;
					}
					/*
					if (current_spike_chip_id == 1 && (current_spike_core_id == 0 || current_spike_core_id == 2)) { //
						nsm_winner_ready = 0;
						break;
					}
					*/
					if (current_spike_chip_id == 1 && current_spike_core_id == 1 &&
						current_spike_row_id >= 0 && current_spike_row_id < NSM_SIZE / 2 * NSM_L &&
						current_spike_col_id >= 0 && current_spike_col_id < NSM_SIZE / 2 * 1) {
						nsm_winner_id = 0;
						nsm_winner_spikes_num += 1;
					}
					if (current_spike_chip_id == 1 && current_spike_core_id == 1 &&
						current_spike_row_id >= 0 && current_spike_row_id < NSM_SIZE / 2 * NSM_L &&
						current_spike_col_id >= NSM_SIZE / 2 * 1 && current_spike_col_id < NSM_SIZE / 2 * 2) {
						nsm_winner_id = 1;
						nsm_winner_spikes_num += 1;
					}
					if (current_spike_chip_id == 1 && current_spike_core_id == 1 &&
						current_spike_row_id >= 0 && current_spike_row_id < NSM_SIZE / 2 * NSM_L &&
						current_spike_col_id >= NSM_SIZE / 2 * 2 && current_spike_col_id < NSM_SIZE / 2 * 3) {
						nsm_winner_id = 2;
						nsm_winner_spikes_num += 1;
					}
					if (current_spike_chip_id == 1 && current_spike_core_id == 1 &&
						current_spike_row_id >= 0 && current_spike_row_id < NSM_SIZE / 2 * NSM_L &&
						current_spike_col_id >= NSM_SIZE / 2 * 3 && current_spike_col_id < NSM_SIZE / 2 * 4) {
						nsm_winner_id = 3;
						nsm_winner_spikes_num += 1;
					}
				}
				if (delta_time > CONSIDERING_PERIOD_CHECK) {
					if (nsm_winner_spikes_num >= NSM_WINNER_SPIKES_NUM_THRESHOULD) { //10
						nsm_winner_ready = 1;
						showNSMWinnerReady(moduleData);
						printf("@@ The NSMs are stable. \n");
					} else {
						nsm_winner_ready = 0;
					}
					end_searching = 1;
...
//	if (nsm_winner_ready == 1) {
//		printf("@@ The NSMs are stable. \n");
//	}

	/********************************************************************************/
	if (chip_id == 3) {
		int64_t new_chip_id = 2;
		int64_t spike_addr = new_chip_id << NEURON_CHIPID_SHIFT | core_id << NEURON_COREID_SHIFT | neuron_id;
		memory.spike_fifo_feature->buffer2d[memory.wr_pointer_feature][0] = spike_addr; //put spike address into the queue
		memory.spike_fifo_feature->buffer2d[memory.wr_pointer_feature][1] = ts; //put spike address into the queue
		memory.wr_pointer_feature = (memory.wr_pointer_feature + 1) % SPIKE_QUEUE_LENGTH;
	}
	...
	if (nsm_winner_ready == 1) {
		...
		for (uint64_t current_rd_pointer = (memory.start_rd_pointer_feature - 1) % SPIKE_QUEUE_LENGTH;
			...
			if (delta_time > 0 && delta_time <= CONSIDERING_PERIOD_CHECK) {
				learnColumnUpdate(nsm_winner_id, current_spike_addr);
			}
			if (delta_time > CONSIDERING_PERIOD_CHECK) {
				printf("@@@ Learn the feature combination. \n");
				learnColumnConfigure(moduleData, nsm_winner_id);
