...
void removeNotReadySignal(caerModuleData moduleData) {
	if (not_ready_added == 1) {
		int64_t pre_row_id, pre_col_id, post_row_id, post_col_id, post_neuron_id;
		...
		for (post_row_id = 0; post_row_id < FEATURE_LAYER_EI_L; post_row_id++) {
			for (post_col_id = 0; post_col_id < FEATURE_LAYER_EI_W; post_col_id++) {
				for (pre_col_id = 0; pre_col_id < 4-1; pre_col_id++) { //FEATURE_LAYER_EI_W
					pre_neuron_addr = feature_layer_1_in[FEATURE_LAYER_EI_L-1][pre_col_id];
