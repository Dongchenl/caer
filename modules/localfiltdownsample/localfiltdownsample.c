...
static void caerLocalFiltDownsampleModuleConfig(caerModuleData moduleData) {
	...
	if (newRun && !state->run) {
	    state->run = true;
	    caerDeviceConfigSet(state->eventSourceModuleState, DYNAPSE_CONFIG_LOCALFILT, 0, true);
	    printf("%d\n", DYNAPSE_CONFIG_LOCALFILT);
	    int foo = 1337;
	    caerDeviceConfigGet(state->eventSourceModuleState, DYNAPSE_CONFIG_LOCALFILT, 0, &foo);
	    caerLog(CAER_LOG_NOTICE, __func__, "Start running got back %d", foo);
	} else if (!newRun && state->run) {
	    state->run = false;
	    caerDeviceConfigSet(state->eventSourceModuleState, DYNAPSE_CONFIG_LOCALFILT, 0, false);
	    int foo = 1337;
	    caerDeviceConfigGet(state->eventSourceModuleState, DYNAPSE_CONFIG_LOCALFILT, 0, &foo);
	    printf("%d\n", DYNAPSE_CONFIG_LOCALFILT);
	    caerLog(CAER_LOG_NOTICE, __func__, "Start running got back %d", foo);
	...
	if (newSetCams && !state->setCams) {
	    state->setCams = true;
	    caerDeviceConfigSet(state->eventSourceModuleState, DYNAPSE_CONFIG_CHIP, DYNAPSE_CONFIG_CHIP_ID, 12);
	    caerLog(CAER_LOG_NOTICE, __func__, "Start cams");
	    for (uint32_t i = 0; i < 1024; i++) {
		caerDynapseWriteCam(state->eventSourceModuleState, i%256, i, 0, DYNAPSE_CONFIG_CAMTYPE_F_EXC);
	...
	if (state->CorrelationThreshold != newCorrelationThreshold) {
	    caerDeviceConfigSet(state->eventSourceModuleState, DYNAPSE_CONFIG_LOCALFILT, 3, newCorrelationThreshold);
